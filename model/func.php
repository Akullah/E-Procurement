...
{
	extract($data);
	$sql = "INSERT INTO supplier (idno, fname, lname, phone, c_name, c_address, c_email, c_cert, c_pin, c_profile, reg_date) VALUES (\"$idnumber\",\"$first_name\",\"$last_name\",\"$phone\",\"$company_name\",\"$company_address\",\"$company_email\",\"$cert\",\"$pin\",\"$profile\",\"$reg_date\")";
...
function getAllSuppliers($conn,$criteria)
{
	$sql = "SELECT * FROM supplier WHERE state=$criteria";
	return fetchData($conn,$sql);
}

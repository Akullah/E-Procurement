	...
	if ($loginDetails) {
		...
		if ($password == $loginDetails['password']) {
			if ($loginDetails['state'] == 1) {
				...
				echo "
				        <script>
				            var \$toastContent = \$('<span> All details provided are ok but your account is still inactive.</span>').add(\$('<button onclick=\"resetPass()\" class=\"btn-flat toast-action\"><i class=\"fa fa-exclamation-triangle yellow-text\"></i></button>'));
				            Materialize.toast(\$toastContent, 4000);
				        </script>
					";die();
			}
